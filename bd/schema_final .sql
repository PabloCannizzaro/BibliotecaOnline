-- Biblioteca Digital: esquema final compatible con MySQL 8 / MariaDB
-- Importar primero este archivo, luego seed_final.sql y por ultimo queries_test.sql.
-- Los indices compuestos incluidos al final de cada tabla optimizan las consultas
-- del panel administrador (stock, ventas y prestamos vencidos).
...
CREATE TABLE IF NOT EXISTS copies (
  ...
  FOREIGN KEY (book_id) REFERENCES books(book_id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  UNIQUE KEY ux_copies_copy_book (copy_id, book_id),
...
CREATE TABLE IF NOT EXISTS loan_items (
  ...
  FOREIGN KEY (loan_id) REFERENCES loans(loan_id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  -- La FK compuesta evita que un prestamo relacione un ejemplar con otro libro.
